...
-- Write a query that shows the id, status, and order total for all
-- orders made by 'pclark74@gmail.com'. Use a join to do this.
...
-- Write a query that shows all the melons, quantity, price, all columns
...
-- Challenge: Produce a list of all salespeople and the total amount of
-- orders they've sold, while calculating a 15% commission on all of
-- their orders. Include their given name, surname, the total of all
-- their sales, and their commission. Only report one row per
-- salesperson. Include salespeople who have not made any sales.
-- Requires left join.

SELECT S.first_name, 
       S.last_name, 
       SUM(order_total), 
       SUM(order_total) * 0.15 AS commission
